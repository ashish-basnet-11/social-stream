...
const FriendshipButton = ({ status, onClick, loading }) => {
  ...
  switch (status) {
    case 'friends':
      return (
        <button onClick={() => onClick('remove')} className={`${baseStyles} bg-white text-slate-400 hover:text-rose-500 border border-slate-200 hover:border-rose-100`}>
          <UserX size={16} /> Disconnect
        </button>
      );
    case 'request_sent':
      return (
        <button disabled className={`${baseStyles} bg-slate-100 text-slate-400 cursor-default border border-slate-200 shadow-none`}>
          Pending...
        </button>
      );
    case 'request_received':
      return (
        <button onClick={() => onClick('send')} className={`${baseStyles} bg-indigo-600 hover:bg-indigo-700 text-white animate-pulse`}>
          Accept Invitation
        </button>
      );
    default:
      return (
        <button onClick={() => onClick('send')} className={`${baseStyles} bg-indigo-600 hover:bg-indigo-700 text-white`}>
