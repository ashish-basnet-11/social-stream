          ...
          >
            <div className="flex items-center justify-between px-8 py-6 border-b border-slate-50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black uppercase tracking-tighter italic text-slate-900">Likes<span className="text-rose-600">.</span></h3>
              </div>
                  ...
                  {likers.map((item) => (
                    <Link key={item.user.id} to={`/profile/${item.user.id}`} onClick={() => setShowLikesModal(false)} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-[22px] transition-all group">
                      <div className="flex items-center gap-4">
                        <div className="w-11 h-11 rounded-2xl bg-slate-100 overflow-hidden border border-slate-200 shadow-sm group-hover:border-rose-200 transition-all">
                          {item.user.avatar ? <img src={item.user.avatar} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center bg-rose-50 text-rose-500 font-bold uppercase">{item.user.name[0]}</div>}
                        </div>
                        <div>
                      ...
                      <User size={16} className="text-slate-200 group-hover:text-rose-400 transition-colors" />
                    </Link>
                  ))}
                </div>
              )}
        ...
        <div className="bg-white border border-slate-200 rounded-[32px] p-6 mb-8 shadow-sm animate-in slide-in-from-top-4 duration-300">
          <form onSubmit={async (e) => {
            ...
            <textarea
              ...
              className="w-full bg-slate-50 text-slate-900 p-4 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-rose-500/5 focus:border-rose-500 outline-none transition-all resize-none font-medium text-sm"
              rows="3"
            />
            <div className="flex gap-2 justify-end">
              ...
              <button type="submit" className="px-6 py-2 bg-slate-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-slate-200 hover:bg-slate-900 transition-all active:scale-95">Save Changes</button>
            </div>
          </form>
        </div>
      ) : (
