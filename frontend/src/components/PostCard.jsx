...
import { Heart, MessageCircle, Send, Bookmark, MoreHorizontal, Trash2, CheckCircle2, AlertCircle } from 'lucide-react';
...
const PostCard = ({ post, onUpdate }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Keep the "Deleting" skeleton visible briefly for visual feedback
      setTimeout(() => {
        onUpdate();
      }, 2000);
    } catch (err) {
    ...
    return (
      <div className="relative">
        <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[32px] p-12 mb-8 flex flex-col items-center justify-center transition-all duration-700 scale-95 opacity-50">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-rose-500 animate-pulse text-center">
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-center text-slate-400 text-xs font-bold uppercase tracking-widest mb-8 leading-relaxed">
              This will permanently remove this transmission from the grid.
            </p>
            <div className="flex flex-col gap-3">
              <button
                onClick={handleDelete}
                className="w-full py-4 bg-rose-500 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-rose-600 transition-all active:scale-95"
              >
                Confirm Delete
              </button>
              <button
        ...
        <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] group/card">
          <div className="p-5 flex items-start justify-between">
            <Link to={`/profile/${post.authorId}`} className="flex items-center gap-3 group/author">
              ...
              <div>
                <h4 className="font-bold text-slate-900 leading-none group-hover/author:text-indigo-600 transition-colors italic uppercase tracking-tighter">
                  {post.author.name}
                </h4>
                    ...
                    {isAuthor && (
                      <>
                        <button
                          ...
                          className="text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 px-3 py-2 rounded-xl transition-all"
                        >
                          Edit
                        </button>

                        {/* Updated Delete Button - Text Only */}
                        <button
                          onClick={() => setShowConfirm(true)}
                          className="text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-rose-600 hover:bg-rose-50 px-3 py-2 rounded-xl transition-all"
                        >
                          Delete
                        </button>
                      </>
          ...
          {showComments && (
            <div className="px-6 pb-6 pt-2 bg-slate-50/40 border-t border-slate-100/50">
              <div className="space-y-4 mt-2 max-h-60 overflow-y-auto pr-2 no-scrollbar">
                {comments.length === 0 ? (
                  <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest py-4 text-center italic">No active comments.</p>
                ) : (
                  comments.map((comment) => (
                    <div key={comment.id} className="flex gap-3">
                      <Link to={`/profile/${comment.author.id}`} className="shrink-0">
                        <div className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-400 overflow-hidden shadow-sm">
                          {comment.author.avatar ? <img src={comment.author.avatar} className="w-full h-full object-cover" /> : comment.author.name[0]}
                        </div>
                      </Link>
                      <div className="flex-1 bg-white p-3 rounded-2xl rounded-tl-none border border-slate-100 shadow-sm">
                        <p className="text-[13px] text-slate-700 leading-snug">
                          <Link to={`/profile/${comment.author.id}`} className="font-black mr-2 text-indigo-600 uppercase text-[9px] tracking-tighter hover:underline">
                            {comment.author.name}
                          </Link>
                          {comment.content}
                        </p>
              ...
              <form onSubmit={handleCommentSubmit} className="mt-5 flex items-center gap-3 bg-white p-1.5 pl-4 rounded-[18px] border border-slate-200 focus-within:ring-4 focus-within:ring-indigo-500/5 focus-within:border-indigo-200 transition-all">
                <input
                  type="text"
                  value={newComment}
                  onChange={(e) => setNewComment(e.target.value)}
                  placeholder="Join the thread..."
                  className="flex-1 bg-transparent text-sm text-slate-700 outline-none placeholder:text-slate-300 font-medium"
                />
                <button disabled={!newComment.trim()} className="bg-slate-900 text-white p-2.5 rounded-xl hover:bg-indigo-600 disabled:opacity-20 transition-all active:scale-90">
                  <Send size={14} />
                </button>
              </form>
            </div>
